...
import com.javaex.vo.PersonVo;
	 ...
	 * 
	 * try { // 3. SQL문 준비 / 바인딩 / 실행 // SQL문 준비 String query = ""; query +=
	 * " select person_id, "; query += "	      name, "; query += "        hp, ";
	 * query += "	      company "; query += " from person ";
	 * 
	 * // 바인딩 pstmt = conn.prepareStatement(query);
	 * 
	 * // 실행 rs = pstmt.executeQuery();
	 * 
	 * // 4.결과처리 while (rs.next()) {// 반복 int personId = rs.getInt("person_id");
	 * String name = rs.getString("name"); String hp = rs.getString("hp"); String
	 * company = rs.getString("company");
	 * 
	 * // db에서 가져온 데이터 vo로 묶기 PersonVo personVo = new PersonVo(personId, name, hp,
	 * company); // 리스트에 주소 추가 personList.add(personVo); }
	 * 
	...
	// 등록
	/*
	 * public int personInsert(PersonVo personVo) { int count = -1;
	 * 
	 * this.getConnection();
	 * 
	 * try { // 3. SQL문 준비 / 바인딩 / 실행 // SQL문 준비 String query = ""; query +=
	 * " insert into person "; query += " values(null, ?, ?, ?) ";
	 * 
	 * // 바인딩 pstmt = conn.prepareStatement(query); pstmt.setString(1,
	 * personVo.getName()); pstmt.setString(2, personVo.getHp()); pstmt.setString(3,
	 * personVo.getCompany());
	 * 
	 * // 실행 count = pstmt.executeUpdate();
	 * 
	 * // 4.결과처리 System.out.println(count + "건 등록되었습니다.");
	 * 
	 * } catch (SQLException e) { System.out.println("error:" + e); }
	 * 
	 * this.close();
